package com.cassetete.backend.algocassete;

import com.cassetete.backend.entity.Combination;

import java.util.ArrayList;
import java.util.List;

public class EquationSolver {

    private static final double EPSILON = 1e-6; //one milionth
    private static final double TARGET = 66.0;

    //Check if the result of the mult and div is almost an int (epsilon degree of precision). Hopefully this is where most of the gains come from
    //We could also multithread but I'm running out of time, and 90 ms on an I5 (was 120 before) seems good enough to me
    private boolean isAlmostInteger(double val) {
        return Math.abs(val - Math.round(val)) < EPSILON;
    }

    // this is unit test only so far
    public static double solveEquation(int x1, int x2, int x3, int x4, int x5,
                                       int x6, int x7, int x8, int x9) {
        if (x3 == 0 || x9 == 0) throw new IllegalArgumentException("Division par zéro impossible");
        return x1 + 13.0 * x2 / x3 + x4 + 12 * x5 - x6 - 11 + x7 * x8 / (double)x9 - 10;
    }

    //init the test array, those ints will be swapped aroudn via backtracking
    public List<Combination> generateAllValidCombinations() {
        List<Combination> results = new ArrayList<>();
        int[] digits = {1,2,3,4,5,6,7,8,9};
        permute(digits, 0, results);
        return results;
    }

    private void permute(int[] arr, int index, List<Combination> results) {
        if (index == arr.length) {
            int x1 = arr[0], x2 = arr[1], x3 = arr[2], x4 = arr[3], x5 = arr[4];
            int x6 = arr[5], x7 = arr[6], x8 = arr[7], x9 = arr[8];

            if (x3 == 0 || x9 == 0) return;

            // check if the result is almost an int
            double div1 = 13.0 * x2 / x3;
            if (!isAlmostInteger(div1)) return;

            double div2 = (x7 * x8) / (double) x9;
            if (!isAlmostInteger(div2)) return;

            double total = x1 + div1 + x4 + 12 * x5 - x6 - 11 + div2 - 10;

            if (Math.abs(total - TARGET) < EPSILON) {
                results.add(new Combination(x1, x2, x3, x4, x5, x6, x7, x8, x9));
            }
            return;
        }

        for (int i = index; i < arr.length; i++) {
            swap(arr, i, index);
            permute(arr, index + 1, results);
            swap(arr, i, index);
        }
    }

    private void swap(int[] arr, int i, int j) {
        int tmp = arr[i];
        arr[i] = arr[j];
        arr[j] = tmp;
    }
}